...
export const getHardwareTier = async () => {
  ...
  try {
    ...
    // capable of running Phi-3.5 (standard) or Llama-3-8B
    if (maxMemory >= 2147483648) { // > 2GB buffer
      return { tier: 'high', recommendedModel: 'Phi-3.5-mini-instruct-q4f16_1-MLC' };
    }

    // Tier 2: Standard (Modern Integrated Graphics, e.g., Intel Iris Xe)
    // Capable of running Llama-3.2-3B or Phi-3
    if (maxMemory >= 1073741824) { // > 1GB buffer
      return { tier: 'medium', recommendedModel: 'Phi-3-mini-4k-instruct-q4f16_1-MLC' };
    }

    // Tier 1: Low Power (Older Intel UHD, Mobile)
    // Strictly limited to tiny models
    return { tier: 'low', recommendedModel: 'Llama-3.2-1B-Instruct-q4f16_1-MLC' };
